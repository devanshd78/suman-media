...
import { getFeaturedInsights } from "@/sanity/lib/data";
import type { CmsFeaturedInsight } from "@/types/cms";
...
/*
 * Server-rendered with a fixed locale, so the output is deterministic —
 * no server/client locale drift.
 */
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 p-6">
        ...
        <h3
          className={`${inter.className} text-lg font-semibold leading-6 text-black transition-colors group-hover:text-[#8F6C1A]`}
...
export async function InsightsSection() {
  const posts = (await getFeaturedInsights()).slice(0, 3);

  // No hardcoded fallback content: this section appears on the homepage
  // once posts are published in Sanity Studio.
  if (posts.length === 0) {
    return null;
  }
