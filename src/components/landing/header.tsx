...
export function Header() {
  ...
  return (
    <header
      className={`z-50 flex w-full max-w-[90rem] items-center justify-between rounded-none ${
        isLandingPage
          ? "absolute left-1/2 top-0 -translate-x-1/2 px-4 py-3 sm:px-8"
          : "relative mx-auto h-[5.75rem] border-b border-black/10 bg-white px-5 sm:px-8 lg:px-[3.5rem]"
      }`}
    >
      ...
      <div
        className={`flex items-center ${
          isLandingPage ? "gap-3 lg:gap-5" : "gap-5 lg:gap-10"
        }`}
      >
        <nav
          aria-label="Primary navigation"
          className={`hidden items-center lg:flex ${
            isLandingPage ? "gap-5 xl:gap-7" : "gap-8 xl:gap-12"
          }`}
        >
          {navItems.map((item) => {
            const isActive =
            ...
            return (
              <Link
                ...
                className={`inline-flex items-center gap-1 transition-colors ${
                  isLandingPage
                    ? `text-[0.6875rem] hover:text-white ${
                        isActive ? "font-bold text-white" : "font-medium text-white/80"
                      }`
                    : `text-base hover:text-black ${
                        isActive ? "font-bold text-black" : "font-normal text-[#969696]"
                      }`
                }`}
              >
        ...
        <Link
          href="/contact"
          className={`inline-flex items-center justify-center gap-2 bg-[#8a6a0a]/85 font-semibold text-white transition-colors hover:bg-[#9a7810] ${
            isLandingPage
              ? "min-h-10 rounded-[2rem] px-4 py-2 text-xs backdrop-blur-sm sm:px-5"
              : "h-[3.625rem] rounded-2xl px-6 text-lg"
          }`}
        >
          <span>Contact us</span>
          <ArrowUpRightIcon className={isLandingPage ? "h-4 w-4" : "h-5 w-5"} />
        </Link>
      </div>
    </header>
  );
